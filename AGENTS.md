...
├── dotfiles/              # dotfiles 配置
│   ├── setup-msys2.sh     # Windows(MSYS2) 部署脚本
│   ├── setup-wsl-*.sh     # 各 WSL 发行版部署脚本
│   ├── setup-dotfiles.sh  # 通用 Stow 部署脚本 (搭配 dotfiles-*.conf)
│   ├── dotfiles-*.conf    # Stow 清单
│   ├── [软件名]/          # 通用软件配置
│   ├── MSYS2/             # Windows(MSYS2) 环境专用软件配置
│   │   └── [软件名]/
│   └── WSL-<发行版>/      # WSL 各发行版专用软件配置
│       └── [软件名]/
├── Container/             # 容器配置
│   ├── README.md          # 容器环境规范
```
