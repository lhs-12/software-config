...
```bash
# 主要组合: 黑体 -> MiSans ; 宋体 -> 霞鹜文楷 ; 等宽 -> Iosevka Term ; nerd symbols
# 合成字体备用: Sarasa Gothic, Maple
# Noto字体: emoji, CJK异体字, 小语种...
# GB18030-2022 标准: MiSans L3 , 霞鹜文楷GB
# Unicode 17.0 标准, 选一个兜底: 天珩字庫, 遍黑体, 文津宋体, 字雲字体...
...
# pacman -S --needed nvidia-open nvidia-utils lib32-nvidia-utils

# 2.2 AMD
# https://wiki.archlinux.org/title/AMDGPU
# AMD 不需要自己安装驱动, 已经由linux-firmware和mesa提供
# 仅需加上AMD的vulkan(图形计算API接口)
pacman -S --needed mesa lib32-mesa xf86-video-amdgpu vulkan-radeon lib32-vulkan-radeon

# 2.3 Intel
# https://wiki.archlinux.org/title/Intel_graphics
# pacman -S --needed mesa lib32-mesa vulkan-intel lib32-vulkan-intel

# 3. 硬件编解码: https://wiki.archlinux.org/title/Hardware_video_acceleration

# 3.1 NVIDIA: 由nvidia-utils和libva-nvidia-driver提供
# pacman -S libva-nvidia-driver
# 还可以替换为nvidia-vaapi-driver，据wiki说功耗更低(但这个包在archlinuxcn)
...
# 其他:
# 混合模式下用独显运行: KDE 开始菜单右键编辑应用程序在高级页面设置用独显运行. 具体看文档
# 显卡切换: KDE 桌面右键进编辑模式, 组件商店下载Optimus GPU Switcher. 具体看文档
```
...
# 插件: 沉浸式翻译, Tampermonkey, AdGuard, Gesturefy, KeePassXC, Vimium, Plasma Integration
# 添加搜索引擎:
