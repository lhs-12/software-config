...
sudo pacman -S --needed wezterm firefox speech-dispatcher keepassxc kcalc kfind kcolorchooser ktorrent gwenview easyeffects haruna

# 截图/贴图
# 截图全局快捷键: ~/.config/kglobalshortcutsrc中定义(dotfiles管理)
# 贴图置顶规则: ~/.config/kwinrulesrc 中定义(dotfiles管理)
# 快捷键手动操作参考: 系统设置 -> 键盘 -> 快捷键 -> 添加命令, 设置F1为全局截图按键
# flameshot功能不完善, 暂时改用screenshot-kde脚本
# sudo pacman -S --neededflameshot
sudo pacman -S --needed wl-clipboard qt6-webengine python-pyqt6-webengine
