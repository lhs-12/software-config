...
完整顺序: packages 装 git/GCM → dotfiles 交付 ~/.gitconfig(credential helper) →
pre-tools hook 装 gh 并登录 → mise install 批量下载时取到 token.
mise 取 token 优先级: gh hosts.yml 直读 > git credential fill(GCM);
因 gh 登录时检测到 GCM 会把 token 存入 keyring 而非 hosts.yml, 所以 ~/.gitconfig 必须在登录前就位
