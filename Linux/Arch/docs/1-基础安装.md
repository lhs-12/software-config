...
# 可选: 安装完后关闭CN源, 仅需要时打开(CN源依赖有时来不及和官方同步, 可能出问题)
# vim /etc/pacman.conf # 注释掉 [archlinuxcn] 部分
...
# 方法2: 使用btrfs-assistant命令行
su - # 需要切换到root用户
btrfs-assistant -l # 查看快照id
btrfs-assistant -r 快照id # 执行回滚
reboot # 重启, 完成快照回滚

# 示例: 从 GRUB 菜单的快照启动项进入系统并回档
# 用btrfs-assistant做回档, 需要root用户登录, GUI或命令行都行
# 注: GRUB 进快照环境的/目录是快照的根不是原系统的根, snapper读不到/.snapshots, 不用snapper
# 
# 如果无法从快照启动项进入系统:
# 1. 设置覆盖文件系统(overlayfs)
#   设置一个overlayfs在内存中创建一个临时可写的类似live-cd的环境, 否则可能无法正常从快照启动项进入系统
# 2. 在HOOKS里添加grub-btrfs-overlayfs
#   vim /etc/mkinitcpio.conf # HOOKS末尾加入项目: HOOKS= (...grub-btrfs-overlayfs)
# 3. 修改了开机配置, 需要重新生成initramfs
#   mkinitcpio -P
# 4. reboot 重启电脑
# 
# 重启后进入 GRUB 界面, 选快照, 选 "vmlinuz-linux & initramfs-linux.img"
# 若没进图形界面, 且卡住, 按 Ctrl + Alt + F2 (切换到另一个 TTY), 输入root账号登录
# 然后使用 btrfs-assistant -r 命令做回滚并重启

# 方法3: btrfs-assistant图形界面(可正常进入桌面环境时用)
# 1. 创建配置
