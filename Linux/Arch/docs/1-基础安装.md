...
6. [Linux-Notes](https://github.com/duanluan/linux-notes)
...
> Live 环境异常时, 可修改引导参数临时规避:
>
> 1. ISO 引导程序通常用 `grub` 或 `systemd-boot`, 选中启动项后按 e 进编辑模式, 在合适位置追加参数
> 2. 部分带图形界面的 ISO 直接提供兼容模式等启动选项, 选了即用
> 3. 常用参数: N卡驱动卡死加 `nomodeset`(禁用图形驱动); MCE 日志刷屏加 `mce=off`
...
# 可以在其他电脑 openssh 连接过去, 复制命令执行
# systemctl status sshd # 如果还没启动就start一下
# passwd # 给 iso live 环境设置一个临时密码
# ip addr # 查看地址
# ssh root@<ip地址> # 从其他电脑执行远程连接
...
# 配置镜像源
reflector -a 24 -c cn -f 10 --protocol https --sort score --save /etc/pacman.d/mirrorlist --v # 自动更新源
# 参数: 最近24小时更新过的;中国镜像;速度最快10个;仅https协议;下载质量评分排序;保存地址;过程可视化
...
#    注: 当前方案ESP挂载在/efi, 只放EFI文件和可能的GRUB模块, 200M足够
#    若ESP挂载在/boot且内核也装进去, 则需要1G以上
...
# ESP挂载在/efi而非/boot的原因: /boot放Btrfs便于快照, 但ESP必须是FAT32, 两者冲突.
# 引导加载器选GRUB而非systemd-boot: systemd-boot 要求内核/initrd等文件放ESP, 无法快照.
#
# GRUB有个问题: 启动时需要写入grubenv来记录上次选了哪个启动项, 但Btrfs是写时复制(CoW)文件系统,
# 直接写入普通文件不稳定, 所以GRUB无法在Btrfs上直接保存grubenv, 导致记忆启动项功能失效.
# 因此需要把GRUB装进ESP(FAT), FAT上可以直接写入, 没有CoW问题.
# GRUB 2.14开始, 利用Btrfs文件系统头部的保留区域存储grubenv, 绕过了这个限制, 可以把GRUB装在Btrfs上了.
#
# 因此ESP挂载在/efi后, GRUB有两种安装方式:
# 方案1 (传统): GRUB模块装进ESP(/efi/grub), /boot/grub做符号链接指向它
#   - 优点: 最简单, 配置少; grubenv在FAT上可直接写入; 任何GRUB版本都支持
#   - 缺点: grub.cfg在FAT上, 不能被快照回档, 回档后grub配置内容可能和系统状态不一致导致异常
# 方案2 (推荐): GRUB模块装到/boot/grub(Btrfs), 通过grub-editenv在Btrfs上初始化grubenv
#   - 优点: grub.cfg和grubenv都可被快照回档; 配置简单, 无需存根、符号链接或修改grub-btrfs配置
#   - 缺点: 需要GRUB 2.14或更新版本; 不适用于LVM, RAID, 加密分区
#
# 两种方案的ESP空间有200M就够, 因为ESP里是EFI文件和GRUB模块, 不包含 /boot 的大文件
...
nvim /etc/hosts # 写入hosts内容(第三行的名称要和主机名一致)
# 127.0.0.1   localhost
# ::1         localhost
# 127.0.1.1   arch.localdomain arch

ln -sf /usr/share/zoneinfo/Asia/Shanghai /etc/localtime # 设置时间时区
# 或用 timedatectl set-timezone Asia/Shanghai
...
useradd -m -G wheel 用户名 # -m创建home目录, -G将wheel设为附加组(主组保持用户私有组)
passwd 用户名 # 设置密码
EDITOR=nvim visudo # 编辑权限: 取消注释 %wheel ALL=(ALL:ALL) ALL

# 可选: 配置 sudo 免密规则 (示例: pacman 免密)
# /etc/sudoers.d/ 是 sudo 的规则文件, 可拆分多个, 要求权限 440 或更严格
# cat << EOF > /etc/sudoers.d/10-nopasswd
# %wheel ALL=(ALL:ALL) NOPASSWD: /usr/bin/pacman
# EOF
# chmod 440 /etc/sudoers.d/10-nopasswd

# 设置安装引导程序(双系统)
# grub: 引导加载器(bootloader), 被UEFI加载后接管引导流程, 显示多系统菜单, 引导加载Linux
# efibootmgr: 管理/修改 UEFI 启动项
# os-prober: 检测硬盘中的其他操作系统 (自动挂载检测, 无需手动挂载)
# fuse3: 用户态文件系统接口(协助 os-prober 找 windows)
pacman -S grub efibootmgr os-prober fuse3

# 执行GRUB引导安装命令(grub-install)
# --- GRUB方案1: GRUB模块装进 /efi/grub (ESP) ---
# 安装GRUB模块到/efi/grub, 生成引导文件到EFI分区(/efi/EFI/ARCH/grubx64.efi), 在主板存储芯片(NVRAM)生成启动项
# grub-install --target=x86_64-efi --efi-directory=/efi --boot-directory=/efi --bootloader-id=ARCH
# 命令参数: --target: 指定架构 ; --efi-directory: ESP 位置 ; --boot-directory: GRUB 模块安装目录 ; --bootloader-id: 设置启动项名称
# 将GRUB默认安装位置/boot/grub软链接到/efi/grub
# ln -sf /efi/grub /boot/grub
# --- GRUB方案2: GRUB模块装到 /boot/grub (Btrfs) ---
# 不使用 --boot-directory 参数, GRUB 模块默认安装至 /boot/grub
grub-install --target=x86_64-efi --efi-directory=/efi --bootloader-id=ARCH

# 可选: 安装回退引导
#
# 【UEFI 启动机制】固件按 BootOrder 顺序逐个尝试条目, 成功即停. BootOrder 就是 BIOS 设置界面里可编辑的启动列表, 其中混合排列着两类条目:
#   · 文件条目: 指向"某磁盘/分区/具体 .efi 文件", 由安装器写入 NVRAM (如 ARCH、Windows Boot Manager)
#   · 设备条目: 固件自带, 只指向设备类别 (如 USB HDD、ATA HDD、NVMe), 启动时在该类设备上寻找回退路径
#
# 【回退路径】`\EFI\BOOT\BOOTx64.EFI` (UEFI 规范定义, 全称 Fallback Boot / Removable Media Boot Path).
#   它不属于任何文件条目: 文件条目只会加载自己指向的那一个 .efi, 指向的文件没了就试下一条目,
#   不会回头找回退路径 (个别主板会额外尝试, 属固件扩展). 回退路径只被两种情况用到:
#   ①设备条目启动时 (在枚举到的该类设备上找 BOOTx64.EFI); ②BootOrder 全部失败后的平台恢复阶段.
#
# 【安装回退引导作用】在设备 ESP 生成回退路径文件, 让设备在没有文件条目可依赖时仍能通过设备条目启动
#   比如移动硬盘换电脑 (固件按设备条目处理)、BIOS 更新/重置清空或损坏了文件条目.
#   注意: 文件条目指向的 \EFI\ARCH\grubx64.efi, 与 \EFI\BOOT\BOOTx64.EFI 是两个独立文件, 装了回退引导也不会在"ARCH 条目失效"时自动顶替
#   例: Arch 装在 HDD1 且 HDD1 装了回退引导 → 列表里同时有 "Arch"(文件条目) 和 "ATA HDD/NVMe"(设备条目) 两个入口.
#       若 Arch 条目失效 → 试下一条 (可能是 Windows); 只有轮到覆盖 HDD1 的设备条目时, 才轮到 HDD1 的回退引导
#       多个 ESP 都有回退引导时, 设备条目内部先枚举到哪个设备由固件决定, 一般是最先尝试到的那个.
#
# 适合安装的情况: 1. 系统在移动硬盘, 换电脑时主板没有它的文件条目 ; 2. 主板存在 NVRAM 兼容性问题 ; 3. 担心更新或重置主板 BIOS 导致文件条目被清空
# 注意: 执行前要先检查该文件是否已存在, 因为如果共享 ESP 的其他系统也在该路径写了回退引导文件, 再执行会覆盖
# 若没有安装回退引导, 又遇上主板升级导致文件条目丢失等情况导致启动不正常, 需要使用U盘 Live 环境排查修复
# grub-install --target=x86_64-efi --efi-directory=/efi --boot-directory=/efi --removable --no-nvram # 安装命令 (GRUB方案1)
grub-install --target=x86_64-efi --efi-directory=/efi  --removable --no-nvram # 安装命令 (GRUB方案2)
# --removable: 将引导安装至回退路径 ; --no-nvram: 不保存在 NVRAM

# 修改GRUB配置
nvim /etc/default/grub
# 启动项记忆: GRUB_DEFAULT=0 改 =saved, 取消 GRUB_SAVEDEFAULT=true 的注释
# 开机日志: GRUB_CMDLINE_LINUX_DEFAULT, 去掉 quiet, loglevel 改为 5 (方便排错)
# 禁用watchdog: GRUB_CMDLINE_LINUX_DEFAULT 添加 nowatchdog, 和 modprobe.blacklist=对应模块(AMD用sp5100_tco;Intel用iTCO_wdt)
# 允许os-prober搜索其他系统: 取消 GRUB_DISABLE_OS_PROBER=false 的注释
# 注: watchdog是死机时自动重启的机制, 桌面用户通常用不到, 禁用可提高开关机速度
# 如果机器硬件故障(如内存坏块)导致 MCE 日志不停刷屏影响操作, 可临时在 GRUB_CMDLINE_LINUX_DEFAULT 添加 mce=off (禁用 Linux MCE 处理), 然后重新生成 grub.cfg

# 生成grub菜单
grub-mkconfig -o /boot/grub/grub.cfg

# --- 仅GRUB方案2执行: start ---
# 初始化 Btrfs 环境块, 让 GRUB 能在 Btrfs 上写入 grubenv (启动项记忆)
# 原理: 利用 Btrfs 文件系统头部的保留区域存储环境块, 避免 CoW 文件系统写入不稳定的问题
# 环境块位置/大小固定保留, 不会被 CoW 挪动或普通写入覆盖; 如果 savedefault 异常可用 grub-editenv create 重置
grub-set-default 0 # 初始化环境块顺带设置默认启动项为第0项
# 也可以执行 grub-editenv - set saved_entry=0
grub-editenv - list # 验证: 应有 env_block=512+1, 表示写入文件系统头部固定区域
# --- 仅GRUB方案2执行: end   ---

# GRUB方案1迁移到方案2指南
# 1. 删除配置
# sudo rm -rf /efi/EFI/ARCH /efi/grub /boot/grub # 清除旧配置
# efibootmgr # 查看启动项
# sudo efibootmgr --delete-bootnum -b [启动项编号] # 删除启动项
# 2. 重新安装配置
...
# 编辑 /etc/default/grub , 改 GRUB_THEME="/usr/share/grub/themes/breeze/theme.txt"
# grub-mkconfig -o /boot/grub/grub.cfg
...
# 选装: 额外磁盘格式支持: FAT, exFAT, F2FS, 光盘格式, XFS . 比如磁盘管理工具 gparted 能用到
# pacman -S dosfstools exfatprogs f2fs-tools udftools xfsprogs
...
# 当仓库和AUR的软功能不正常时,可尝试换成Flatpak版本软件
pacman -S flatpak
# 设置Flathub源
...
```bash
pacman -S bash-completion fish oh-my-posh trash-cli perl-image-exiftool
pacman -S openssh wget man-pages man-db less ouch zip unzip 7zip pacman-contrib bind traceroute
pacman -S stow tmux lsd bat bat-extras zoxide dust yazi tldr btop git-delta
pacman -S neovim fzf fd ripgrep lazygit jq imagemagick resvg poppler mdbook mediainfo tree-sitter-cli
```
...
# snap-pac: pacman命令钩子, 自动创建快照, 快照数量较多. 不建议装
...
# snapper 命令开放 wheel 组免 sudo 操作快照
# ALLOW_GROUPS: 允许操作快照的用户组 ; SYNC_ACL: 用户组访问权限以ACL(访问控制列表)形式同步到快照目录(.snapshots)
snapper -c root set-config ALLOW_GROUPS="wheel" SYNC_ACL="yes"
snapper -c home set-config ALLOW_GROUPS="wheel" SYNC_ACL="yes"
# 新建快照
...
# sudo nvim /etc/fstab        # 编辑fstab, 删掉swap相关的内容
...
sudo nvim /etc/systemd/zram-generator.conf # 注: zram-size 是压缩前的大小
# [zram0]
# zram-size = ram / 2
...
# 换内核后显卡驱动需要重新编译(仅N卡, A/I卡驱动在内核内, 无需处理)
# N卡的情况由于装的 nvidia-open-dkms 会自动重新编译, 所以也不用操作
# 重新生成grub
sudo grub-mkconfig -o /boot/grub/grub.cfg
# 重启
