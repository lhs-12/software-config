...
> 若只有 NVIDIA 显卡或双显卡但开启了N卡直连, 则进入 Live 环境时可能因缺少显卡驱动卡死. 解决方法:
> 1. 找带驱动的 ISO, 或 ISO 可能提供 `compatible` 或 `nomodeset` 选项
> 2. ISO 引导程序通常用 `grub` 或 `systemd-boot`, 按 E 进编辑模式在合适位置添加 `nomodeset` 参数
