...
# --target: 指定架构 ; --efi-directory: ESP 位置 ; --boot-directory: GRUB 安装目录 ; --bootloader-id: 设置启动项名称
# 该命令会在 /efi/EFI/ARCH 目录生成 grubx64.efi 文件, 生成安装目录 /efi/grub, 并在主板存储芯片(NVRAM)生成对应的启动项文件
grub-install --target=x86_64-efi --efi-directory=/efi --boot-directory=/efi --bootloader-id=ARCH

# 可选: 安装回退引导
# UEFI 有一个默认启动路径`EFI/BOOT/BOOTx64.EFI`, 称为 Fallback Boot 或 Removable Media Boot Path, 是为了U盘等可移动设备设计的
# 若无法通过 NVRAM 正常启动, 比如: NVRAM 被清空(更新 BIOS, 恢复默认设置等) ; 启动项损坏 ; 没插入启动项对应的移动硬盘 ...,
# 则 UEFI 固件会尝试回退路径, 如果该文件存在, 则仍可启动系统.
# 适合安装的情况: 1. 系统装在移动硬盘 ; 2. 主板存在 NVRAM 兼容性问题 ; 3. 担心更新或重置主板 BIOS 后 NVRAM 被清空
# 注意: 要先检查该文件是否已存在, 如果同硬盘分区的 Windows 或其他系统也使用该路径作为回退启动文件, 会覆盖导致问题
# 安装命令: grub-install --target=x86_64-efi --efi-directory=/efi --boot-directory=/efi --removable --no-nvram
# --removable: 将引导安装至回退路径 ; --no-nvram: 不保存在 NVRAM
# 若没有安装回退引导, 又遇上主板升级导致 NVRAM 丢失等情况导致的启动不正常, 需要使用U盘 Live 环境排查修复
...
# 注: watchdog作用是死机时自动重启, 个人用户没用, 禁用可提高开关机速度
...
# pacman -S dosfstools exfat-utils f2fs-tools udftools xfsprogs
