...
locale-gen # 生成本地化配置
echo "LANG=en_US.UTF-8" > /etc/locale.conf # 设置系统语言英文(暂时别设中文, tty会乱码)

passwd root # 设置root密码

vim /etc/environment
# 设置默认编辑器 EDITOR=vim (neovim填nvim), 该配置重启才生效
...
grub-mkconfig -o /efi/grub/grub.cfg # 生成grub菜单
...
pacman -S openssh wget man-pages man-db less zip unzip 7zip pacman-contrib
pacman -S stow tmux lsd bat bat-extras zoxide dust yazi tldr bottom git-delta
pacman -S neovim fzf fd ripgrep lazygit jq imagemagick resvg poppler mdbook
```
...
# yay: Arch用户仓库
# flatpak: 应用沙盒系统
pacman -S yay flatpak
# 更换flatpak国内源
# 即使用了镜像站 也需要让网络能访问flathub官方仓库, 才能下载所有包
...
# 构建一次grub配置文件, 用于生成grub菜单的snapshot入口
grub-mkconfig -o /efi/grub/grub.cfg
#
# 3. 如果无法进入快照
# 3.1 设置覆盖文件系统(overlayfs)
# 设置一个overlayfs在内存中创建一个临时可写的类似live-cd的环境, 否则可能无法正常从快照启动项进入系统
# 3.2 在HOOKS里添加grub-btrfs-overlayfs
vim /etc/mkinitcpio.conf # HOOKS末尾加入项目: HOOKS= (...grub-btrfs-overlayfs)
# 3.3 修改了开机配置, 需要重新生成initramfs
mkinitcpio -P
# 3.4 重启电脑
...
# 示例: 回滚 root 子卷的快照(正常进入系统环境时)
snapper -c root list # 查看快照, 记住快照号
snapper -c root undochange n..0 # 回滚到n号快照状态(undochange不稳定, 推荐用rollback)

# 示例: 从 GRUB 环境启动回滚到某个快照(用于系统崩溃)
# 开机, 进入 GRUB 界面, 选快照, 选 "vmlinuz-linux & initramfs-linux.img"
# 若没进入图形界面, 按 Ctrl + Alt + F2 (切换到另一个 TTY), 输入root账号登录
btrfs-assistant -l # 查看快照id
# 注: GRUB 进快照环境的/目录是快照的根不是原系统的根, snapper读不到/.snapshots, 因此不用snapper
